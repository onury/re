...
(function () {
    ...
    if (typeof window === 'undefined') {
        re = require('../dist/re.min');
    } else {
        re = window.re;
    }

    // DO NOT MODIFY/MOVE either the contents or the format of this string.
    ...
    describe('re', () => {
        ...
        it('.eachRight()', () => {
            ...
            re(originalRegexp).eachRight(strTest, (matches, index, regexp) => {
                result.push(matches[0]);
                // break iteration @ 2nd from top, 6th from bottom
        ...
        it('.eachInverse()', () => {
            let m = [];
            re(/\d/i).eachInverse('foo1baz2bar', (matches, index, regexp, charIndex) => {
                m.push(matches[0]);
            });
            expect(m).toEqual(['foo', 'baz', 'bar']);

            m = [];
            re(/\d/i).eachInverse('1baz2bar', (matches, index, regexp, charIndex) => {
                m.push(matches[0]);
            });
            expect(m).toEqual(['baz', 'bar']);

            m = [];
            re(/\d/i).eachInverse('foo1baz2', (matches, index, regexp, charIndex) => {
                m.push(matches[0]);
            });
            expect(m).toEqual(['foo', 'baz']);
        });
